...
#   return(cusflag)
# }
cus_flag_func = function(cusdt_all,orderdt_all,enddate)   #定义三个参数的函数
{
  cusdt = subset(cusdt_all,create_date <= enddate)
  orderdt = subset(orderdt_all,create_date <= enddate)
  cusflag = data.frame()  
  cusnos = na.omit(cusdt$id)  #所有客户id
  for (no in cusnos){  
    cusorder=subset(orderdt,orderdt$customer_id==no)
    if(nrow(cusorder)==0) 
    {
      flag="登记" 
      r=data.frame(cusno=no,flag=flag,lastdate=as.Date(NA))
      cusflag=rbind(cusflag,r)
      next
    }
    dates=as.Date(cusorder$create_date[order(as.Date(cusorder$create_date),decreasing = F)])
    firstdate=dates[1]
    lastdate=dates[length(dates)]
    num=length(dates)
    if(num == 1 && (enddate-firstdate)<90&&(enddate-lastdate)<40 )
    {
      flag = '体验用户'
    }
    else if(num ==1 && (enddate-firstdate)<90&&(enddate-lastdate)>=40&&(enddate-lastdate)<90)
    {
      flag = '危险体验用户'
    }
    else if(num >= 2 && (enddate-firstdate)<90&&(enddate-lastdate)<40)
    {
      flag = '激活用户'
    }
    else if(num >= 2 && (enddate-lastdate)>=40&&(enddate-lastdate)<=90 )
    {
      flag = '危险激活用户'
    }
    else if(num == 1 && (enddate-firstdate)>=90 )
    {
      flag = '流失体验用户'
    }
    else if(num >= 2 &&(enddate-firstdate)>=90&& (enddate-lastdate)<40)
    ...
    else if(num >= 2 &&(enddate-lastdate)>=40&&(enddate-lastdate)<=90&&(enddate-firstdate)>=90 )
    {
      flag = '危险用户'
    }
    else if(num >= 2 && (enddate-firstdate)>=90&& (enddate-lastdate)>90)
    {
      flag = '流失用户'
    }
    else
