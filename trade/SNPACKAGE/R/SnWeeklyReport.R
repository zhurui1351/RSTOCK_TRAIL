SnWeeklyReport <- function(priceData,confProp=0.55,conf=0.05,tradeDays=5,prune=5,profitratio=0.2,drawdownratio=-0.1,initEq = 100000,path='')
{
  snrules = SNweeklyRules(priceData,confProp=confProp,conf=conf,tradeDays=tradeDays,prune=prune)
  ...
  for(i in 1:length(traderules))
  {
    ...
    report = backTestBySnRule(traderule$rule,priceData,traderule$buyday,traderule$sellday,short = traderule$short,verbose=FALSE,initEq=initEq)
    ...
    else
    {
      ...
      if(path != '')
      {
        ...
        write(drawdownratio,sumname,append=T)
