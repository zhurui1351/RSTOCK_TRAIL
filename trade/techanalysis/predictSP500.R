...
data.model <- specifyModel(T.ind(GSPC) ~ Delt(Cl(GSPC),k=1:10) 
                           ...
                           +myBB(GSPC)  + myCLV(GSPC) +CMO(Cl(GSPC)) 
                           + EMA(Delt(Cl(GSPC)))  +myVolat(GSPC)  + myMACD(GSPC) 
                            + RSI(Cl(GSPC))  + runMean(Cl(GSPC)) 
                           + runSD(Cl(GSPC)))
...
data.model <- specifyModel(T.ind(GSPC) ~ Delt(Cl(GSPC), k = 1) + myATR(GSPC) + myADX(GSPC) + myEMV(GSPC) + myVolat(GSPC) +
  ...
  if(signals[d]=='b' && !nOs)
  {
    ...
    if(quant > 0)
      orders<-rbind(orders,data.frame(order=c(1,-1,-1),order.type=c(1,2,3),
                                      val=c(quant,market[d,'Close']*(1+exp.prof),market[d,'Close']*(1-max.loss)),
                                      action=c('open','close','close'),posID=c(NA,NA,NA)
                                      ))
    # ii) short positions
  } else if (signals[d]== 's' && !nOs)
  {
    # this is the nr of stocks we already need to buy because of currently opened short postitions
...
# policy 2
policy.2 <- function(signals,market,opened.pos,money,bet=0.2,exp.prof=0.025, max.loss= 0.05)
{
  d<-NROW(market) # this is the ID of today
  orders <- NULL
  nOs <- NROW(opened.pos)
  ...
  if(signals[d]=='b')
  {
    ...
    {
      # this is the money already committed to by stocks because of currently opened short positions
      need2buy <- sum(opened.pos[opened.pos[,'pos.type']==-1,"N.stocks"]) * market[d,'Close']
      quant <- round(bet * (money - need2buy)/market[d,'Close'],0)
      if(quant > 0)
        orders <- rbind(orders,data.frame(order=c(-1,1,1),order.type=c(1,2,3),
...
slide<- function(form,train,test,learner,relearn.step,policy.func,...)
{
  ...
  p<-factor(p,levels=1:3,label=c("s","h","b"),eval.stats(form,train,test,p,policy.func=policy.func))
...
eval.stats <- function(form,train,test,preds,b.t=0.1,s.t=-0.1,...) {# Signals evaluation
   tgtName <- all.vars(form)[1]
...
## warning ,code below will run a very long time
...
VARS$nnetR <- list(linout=T,maxit=750,size=c(5,10), decay=c(0.001,0.01),policy.func=c('pol1','pol2','pol3'))
VARS$nnetC <- list(maxit=750,size=c(5,10),decay=c(0.001,0.01), policy.func=c('pol1','pol2','pol3'))
# main loop
for(td in TODO) {
  assign(td,
         experimentalComparison(
           c(do.call('variants', c(list('single',learner=td),VARS[[td]],
                                       varsRootName=paste('single',td,sep='.'))), do.call('variants',
...
plot(100*yearlyReturn(trade.res@trading$Equity),main='Yearly percentage returns of the trading system') 
abline(h=0,lty=2)
