...
bulllist  = find_bull(ispoint=T,uppoint = 600,downpoint = -200,shindex = na.omit(dou1[,1:4]))
list_l  = bulllist
...
bearlist = find_bear(uppoint = 200,downpoint = -600,shindex = na.omit(dou1[,1:4]),ispoint = T)
list_l  = bearlist
myindex = na.omit(dou1[,1:4])
for(i in 1:(length(list_l )-1))
{
