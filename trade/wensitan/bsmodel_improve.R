...
testdate = substr(as.character(index(to.yearly(pricedata['2013']))),1,4)
...
{
  print(y)
  #starttraindate = as.character(as.numeric(y) - 4)
  ...
  #建模
  model = naiveBayes(leadclflag ~ . -ccisignal,
  ...
  {
    pv = pr[i,]
    tradetime = index(analysedata_test[i,])
    
    numerindex = analysedata_update[tradetime][,2:ncol(analysedata_update)]
