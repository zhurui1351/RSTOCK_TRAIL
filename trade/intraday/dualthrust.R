...
dualthrust <- function(n=3,ks = 0.35,kx = 0.2,initEq = 1000000,minute=1,initDate = '2000-01-01', verbose=F)
{
  #init portofolio
  .blotter <<- new.env()
  symbol = "m_data"
  ...
  {
    #截止昨天，n日内的range
    range = max(daydata[day-1]$HH-daydata[day-1]$LC,daydata[day-1]$LC-daydata[day-1]$LL)
